...
public class Boss extends Monster implements HeroicUnit{
    ...
    public Boss(MassageCallBack m, char tile, Position position, String name, int health_pool, int attack_points, int defense_points, int experience, int vision_range, int ability_frequency) {
        super(m,tile, position, name, health_pool, attack_points, defense_points, experience, vision_range);
    ...
    @Override
    public Tile copy() {
        return new Boss(m,tile,position,name,health_pool,attack_points,defense_points,experience,vision_range,ability_frequency);
    }

    public void SpecialAbility(GameBoard gameBoard){
        Tile player = gameBoard.findPlayer(this, vision_range);
        if (player!=null) {
            m.send(name+" shoots "+player.getName()+" for "+specialAbilityPower()+" damage");
